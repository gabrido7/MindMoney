...
SELECT '=== 8. FK categoria inexistente em transactions (deve FALHAR) ===' AS step;
INSERT INTO transactions (user_id, category_id, description, amount, type, transaction_date)
VALUES (@uid, 999999, 'Categoria fantasma', 100.00, 'saida', '2026-08-10');

SELECT '=== 9. RESTRICT: apagar categoria com transacao (deve FALHAR) ===' AS step;
DELETE FROM categories WHERE id = @cat_alimentacao;

SELECT '=== 10. SET NULL: apagar subcategoria em uso (funciona; a transacao so perde a subcategoria) ===' AS step;
...
SELECT '=== 11. limpeza: apagar conta requer apagar as transacoes primeiro ===' AS step;
...
SELECT '=== 12. confirma limpeza total (tudo 0, exceto templates) ===' AS step;
